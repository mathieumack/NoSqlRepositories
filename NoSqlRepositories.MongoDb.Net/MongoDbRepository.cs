...
namespace NoSqlRepositories.MongoDb
{
    public class MongoDbRepository<T> : RepositoryBase<T> where T : class, IBaseEntity, new()
    {
        ...
        public override INoSqlEntity<T> GetById(string id)
        {
            ...
            var nosqlEntity = new NoSqlEntity<T>(CollectionName, documentElement);
        ...
        public override INoSqlEntity<T> CreateNewDocument(string id)
        {
            ...
            var entity = new NoSqlEntity<T>(this.CollectionName, document);
            return entity;
        }
        ...
        public override InsertResult InsertOne(INoSqlEntity<T> entity, InsertMode insertMode)
        {
            ...
            {
                var existingEntity = GetById(entity.Id) as NoSqlEntity<T>;

                nosqlEntity.SystemCreationDate = existingEntity.SystemCreationDate;
            ...
            try
            {
                try
                {
                    ...
                    collection.InsertOne(nosqlEntity.GetEntityDomain());
        ...
        public override Core.UpdateResult Update(INoSqlEntity<T> entity, UpdateMode updateMode)
        {
            var nosqlEntity = entity as NoSqlEntity<T>;
            if (nosqlEntity == null || nosqlEntity.CollectionName != this.CollectionName)
                throw new InvalidOperationException("the entity was created from an other repository");
            ...
            if (updateMode == UpdateMode.db_implementation)
            {
                ...
                nosqlEntity.SystemLastUpdateDate = updateDate;

                var filter = Builders<T>.Filter.Eq(s => s.Id, entity.Id);

                ReplaceOneResult res = collection.ReplaceOne(filter, nosqlEntity.GetEntityDomain(), new UpdateOptions { IsUpsert = false });
        ...
        {
            return collection.Find(e => !e.Deleted).ToEnumerable().Select(e => new NoSqlEntity<T>(TypeName, e));
        }
