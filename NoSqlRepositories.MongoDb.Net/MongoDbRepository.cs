    ...
    public class MongoDbRepository<T> : RepositoryBase<T> where T : class, IBaseEntity, new()
    {
        ...
        public override InsertResult InsertOne(INoSqlEntity<T> entity, InsertMode insertMode)
        {
            ...
            if (insertMode == InsertMode.erase_existing && Exist(entity.Id))
            {
                ...
                nosqlEntity.DomainEntity.SystemCreationDate = existingEntity.DomainEntity.SystemCreationDate;
            ...
            try
            {
                try
                {
                    ...
                    collection.InsertOne(nosqlEntity.DomainEntity);
        ...
        public override Core.UpdateResult Update(INoSqlEntity<T> entity, UpdateMode updateMode)
        {
            ...
            if (updateMode == UpdateMode.db_implementation)
            {
                ...
                nosqlEntity.DomainEntity.SystemLastUpdateDate = updateDate;

                var filter = Builders<T>.Filter.Eq(s => s.Id, entity.Id);

                ReplaceOneResult res = collection.ReplaceOne(filter, nosqlEntity.DomainEntity, new UpdateOptions { IsUpsert = false });
